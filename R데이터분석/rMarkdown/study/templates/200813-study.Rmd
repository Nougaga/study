...
bh.lm2 <- lm(medv~crim + zn + nox + rm + dis + rad + tax + 
    ptratio + b + lstat, data=bh); summary(bh.lm2)
par(mfrow=c(2,2))
plot(bh.lm2, which=1)  # 선형성(빨간 실선이 0에 가까운 수평선) & 독립성(특정한 모여있는 패턴이 발견되지 않음)
plot(bh.lm2, which=2)  # 정규성(직선에 가깝게 잘 모여있음)
plot(bh.lm2, which=3)  # 등분산성 & 독립성(적절하게 퍼져 있음, 특정 패턴 없음)
plot(bh.lm2, which=4)  # 극단치
